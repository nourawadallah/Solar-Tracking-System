...
void TIMER1_void_FastPWM_ICR_Init(u8 copy_u8_pin) {
    ...
    switch (copy_u8_pin) {
        case OC1A:
            SET_BIT(TCCR1A, 7);   // COM1A1 = 1
            CLEAR_BIT(TCCR1A, 6); // COM1A0 = 0 (non-inverting)
            break;

        case OC1B:
            SET_BIT(TCCR1A, 5);   // COM1B1 = 1
            CLEAR_BIT(TCCR1A, 4); // COM1B0 = 0 (non-inverting)
            break;
    }
}
...
void TIMER1_void_FastPWM_ICR_SetCompare(u8 copy_u8_pin, u16 copy_u16_compare) {
    ...
    {
        case OC1A:
            OCR1A = copy_u16_compare;
            break;

        case OC1B:
            OCR1B = copy_u16_compare;
            break;
    }
}